$(document).ready(function() {
	...
	var root_id = $("#wm_root_id", wordmap).text().toLowerCase();
	var image_path = $("#wm_image_path", wordmap).text();
	// json url
	var url = $("#wm_system_url", wordmap).text()+"item/";// [word]?format=json
	if (!root_word) return ;
	...
	var noimage_path = $("img", infomenu).attr("src");
	var info_synset = $("#wm_info_synset", infomenu);
	...
	var createRoot = function(){
		...
		root = wordarea.addRootNode(root_word, {
			...
			onclick: function() {
				...
				addInfoData(data[root_word].info, root_word);
				return false;
			}
		});
	};
	...
	var addInfoData = function(info, word) {
		if (info !== void 0) {
			$("strong").text(word);

			if (info["id"] !== void 0) {
				$("a[title='details']", infomenu).attr("href", "../"+info["id"]);
			}
			
			if (info["image"] !== void 0) {
				$("img", infomenu).attr("src", image_path + info["image"]+"_320x240.png");
			} else {
				$("img", infomenu).attr("src", noimage_path);
			}

			if (info["Synset"] !== void 0) {
				$.each(info["Synset"], function(i, val){
					info_synset.append(
							$("<a/>").attr("href", "../../wordnet/synset/"+val).text(val)
					).append($("<br />"));
				});
			}
		}
	...
	var addChildren = function(parentnode, parent) {
		...
		$.each(parent.children, function(i, val) {
			...
			wordarea.addNode(parentnode, val.word.toLowerCase(), {
				...
				onclick: function() {
					...
					if (this.children.length > 0) {
						...
						getjson(url + val.id + "/wordmap", function(json, status) {
							wdata = json[val.word];
							addChildren(node, wdata);
							addInfoData(wdata.info, val.word);
						});
					}
					return false;
				}
	...
	// get root_word data
	getjson(url + root_id + "/wordmap", function(json, status){
		data = json;
		createRoot();
