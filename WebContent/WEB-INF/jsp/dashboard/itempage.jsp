...
<c:if test="${itemPage.hasPre}">
		<c:url var="prevUrl" value="/dashboard/${itemType}/1">
...
<c:forEach begin="${startPage}" end="${endPage}" var="pageNo">
    <c:choose>
        ...
        <c:otherwise>
	        <c:url var="pageNumUrl" value="/dashboard/${itemType}/${pageNo}">
...
<c:if test="${itemPage.hasNext}">
	<c:url var="nextUrl" value="/dashboard/${itemType}/${itemPage.totalPages}">
